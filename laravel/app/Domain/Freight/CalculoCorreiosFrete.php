...
namespace App\Domain\Freight;

use App\Domain\Freight\Interfaces\ICalculateFreight;
use App\Exceptions\CorreiosFreteException;
use App\Models\Product;

class CalculoCorreiosFrete implements ICalculateFreight
{
    // Serviço escolhido para o teste
    const COD_SERVICO = 40010;

    public function getTotal($originCEP, $destinyCEP, Product $product, $amount = 1)
    {
        $peso = $product->weight;
        ...
        $correios = "http://ws.correios.com.br/calculador/CalcPrecoPrazo.aspx?nCdEmpresa=&sDsSenha=&sCepOrigem=" . $originCEP . "&sCepDestino=" . $destinyCEP . "&nVlPeso=" . $peso . "&nCdFormato=1&nVlComprimento=" . $comprimento . "&nVlAltura=" . $altura . "&nVlLargura=" . $largura . "&sCdMaoPropria=n&nVlValorDeclarado=" . $valor_declarado . "&sCdAvisoRecebimento=n&nCdServico=" . self::COD_SERVICO . "&nVlDiametro=0&StrRetorno=xml";

        $xml = simplexml_load_file($correios);

        if ($xml->cServico->Erro == '0')
        {
            $valor = (double)$xml->cServico->Valor;
            return $valor * $amount;
        }

        throw new CorreiosFreteException($this->getErrorMessage((string)$xml->cServico->Erro));
    }
